...
def load_obs(cat, seed=1, nproc=1, nmin=10, sps=None):
    """Load the photometry for this galaxy.
    ...
    obs['maggies'] = np.array([flam * 1e-17 * 10**(0.4*48.6) * f.wave_effective**2 / 2.99792458e18
                               for flam, f in zip(cat['FLUX_FLAM'], obs['filters'])])
    obs['maggies_unc'] = np.array([flam * 1e-17 * 10**(0.4*48.6) * f.wave_effective**2 / 2.99792458e18
                               for flam, f in zip(cat['FLUX_FLAM_ERR'], obs['filters'])])
...
def load_model(obs, template_library='delayed-tau'):
    """
...
def main():
    ...
    datadir = os.path.join(os.getenv('HIZEA_PROJECT'), 'hizea-seds')
    catfile = os.path.join(datadir, 'hizea_photo_galex_wise_v1.1.fits')
    cat = Table.read(catfile)
    cat = cat[:1]
    print('Read {} galaxies from {}'.format(len(cat), catfile))

    sps = load_sps()
    
    for gal in cat:
        print('Working on {} at z={:.4f}'.format(gal['SHORT_NAME'], gal['Z']))
        prefix = gal['SHORT_NAME'].lower()
        hfile = os.path.join(datadir, '{}-{}.h5'.format(prefix, args.priors))
        ...
        if args.sedfit:
            ...
            obs, rp = load_obs(gal, seed=args.seed, nproc=args.nproc, sps=sps)
            model = load_model(obs, args.priors)

            print('Nested sampling with {} cores...'.format(args.nproc), end='')
            t0 = time.time()
        ...
        if args.qaplots:
            ...
            png = os.path.join(datadir, '{}-{}-sed.png'.format(prefix, args.priors))
            bestfit_sed(obs, chain=result['chain'], lnprobability=result['lnprobability'], 
                        sps=sps, model=model, seed=1, nrand=100, png=png)

            png = os.path.join(datadir, '{}-{}-corner.png'.format(prefix, args.priors))
